...
Define the one-off Compose backup service that safely stops the local Project Zomboid server through RCON and creates verified offline backups.
...
able to run `docker compose run --rm --no-deps pz-backup` while `pz-server` is running. The service SHALL not
...
- **WHEN** `pz-server` is running and an operator runs `docker compose run --rm --no-deps pz-backup`
...
The root-host backup-and-poweroff workflow SHALL invoke `docker compose run --rm --no-deps pz-backup` from a
